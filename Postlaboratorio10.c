/*
 * Archivo: Postlaboratorio10.c
 * Dispositivo: PIC16F887
 * Autor:Esli Vivar. 
 * Hardware: Valor recibido en el ser?al (RCREG) se muestra en el PORTB
 * 
 * Creado: 7/05/22
 * 
 */
...
{
    if(PIR1bits.RCIF){          // Confirmar recepción de datos 
        RX = RCREG;
    }
...
void main(void) {
    ...
    while(1){
        ...
        pal("\r *************  \r");
        pal("\r Bienvenidx! Selecciona la opción que desas realizar: \r");
        pal(" 1) Leer Potenciometro \r");
        pal(" 2) Desplegar valor ASCII\r");
        pal("\r *************  \r");
        ...
        while (flag) {
            ...
            switch (selection) {
                case ('1'):
                    pal("\r Valor del Potenciómetro:  \r");
                ...
                case ('2'):
                    pal("\r Ingrese el valor que desea mostrar en el PortB: ");
...
void setup (void){
    ...
    TXSTAbits.SYNC = 0;         // Comunicacion ascincrona (full-duplex)
